...
```
if x | == 0 -> "zero" | > 0 -> "positive" | else "negative"
if x { == 0 -> "zero", > 0 -> "positive", else "negative" }
if x {
  == 0 -> "zero",
  > 0 -> "positive",
...
Comparator {
    ...
    type T[A, P] = { compare: (a1: A, a2: A) -> Int, sym: P, .. },
    
    new = [A](compareFn: (a1: A, a2: A) -> Int): T[A, Symbol] -> {
      compare = compareFn
      sym = symbol()
    }
}

type Comparator[A, P] = { compare: (a1: A, a2: A) -> Int },
Set {
    type T[A, P] = { comparator: Comparator[A, P], set: TreeSet[A] },
    
    new = [A, C] = (comparator: 
}

compareAsc = { compare: (s1: String, s2: String) -> ... }
compareDsc = { compare: (s1: String, s2: String) -> ... }

let s1 = Set.new(compareAsc)
let s2 = Set.new(compareDsc)
...
## Default Parameters
Default parameters are tricky because you either need some way of signaling to the function body that an argument's value needs to be ignored (i.e. pass in a null value and replace it) or overloads need to be generated for every combination of the default parameters. Then after deciding on a strategy, you need to be careful about the ordering of the param initialization and need to decide if the default param values can depend on other param values. Instead of dealing with all that, you can say that any labeled param of type `Option.T[A]` can be omitted at the callsite. The compiler will implicitly fill it with an `Option.None` and inside the function body the user can pattern match on the value or call `getOrElse` to choose a default. The downside to this method is that the default value is not shown in the function signature and must instead be documented explicitly. I think that's a worthwhile tradeoff for the relative simplicity in compiler implementation and user semantics. It is very easy to understand, and worst case it can easily be changed in the future by adding an `= None` to the function definitions. The main downside is you have to wrap the arg with `Some`. If there's an interface like Rust's `Into` that has a blanket impl for values, the arg could be automatically converted into an option. Though then that would require every func with defaults to add a bunch of implicit args. Maybe there's a way to simplify that? 

## Equals and HashCode
Some objects that need to interop with Java code need to implement certain methods, e.g. `equals` and `hashCode`. Right now I'm generating a default impl of them that is structural, i.e. all fields are included. This should match the behavior of `value` classes when they're released, in which point I can just remove the generation of them completely. To override eq and hash or to implement a Java interface, named structs need to have their function generated as functions rather than field objects. Or they could be generated as both in those cases, the method on the class just delegates to the field. The issue is the equals signature expects the other arg to be an `Object`, meaning it needs to be coerced into a type before the fields are checked. I could use `invokedynamic` with a type-guarded invocation to the other type to make the check more performant. Honestly it's probably not needed once I move most struct instances to be named types since the normal `instanceof` will be necessary. 

I tried automatically generating a default equals and hashCode for all structs, but this has an issue when two literal structs from different modules are compared. The `instanceof` check fails, so they're never equal. The solution if I wanted to keep the autogeneration is to check if the class implements some `LiteralStructBase` interface and then reflect over the fields to check if they're equal. 
...
* https://dl.acm.org/doi/abs/10.1145/3689746

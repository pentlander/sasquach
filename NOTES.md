...
## Equals and HashCode
Some objects that need to interop with Java code need to implement certain methods, e.g. `equals` and `hashCode`. Right now I'm generating a default impl of them that is structural, i.e. all fields are included. This should match the behavior of `value` classes when they're released, in which point I can just remove the generation of them completely. To override eq and hash or to implement a Java interface, named structs need to have their function generated as functions rather than field objects. Or they could be generated as both in those cases, the method on the class just delegates to the field. 

```
type EqHashWrapper = { equals: (
